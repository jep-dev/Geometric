...
vec4 qconj(vec4 q) { return vec4(q.x, -q.yzw); }
...
Dual dconj(Dual d) { return Dual(qconj(d.u), vec4(-d.v.x, d.v.yzw)); }
// Dual multiplication
...
Dual qdmul(vec4 l, Dual r) { return Dual(qmul(l, r.u), qmul(l, r.v)); }
Dual dqmul(Dual l, vec4 r) { return Dual(qmul(l.u, r), qmul(l.v, r)); }

Dual qdsandwich(vec4 l, Dual r) { return dqmul(qdmul(l, r), qconj(l)); }
